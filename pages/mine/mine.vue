<template>
	<view class="pages">
		...
		<uni-list>
			<uni-list-item title="跳转到一个页,回" link="navigateTo" to="/pages/test/test" clickable />
			<uni-list-item v-for="item in itemList" :key="item.title" :title="item.title" :note="item.note"
				rightText="修改" :thumb="item.thumb" :thumb-size="item.thumbSize" showArrow link="navigateTo"
				:to="item.to" />
		</uni-list>
	</view>
</template>
